...
def create_study_flow_graph(checkpointer_path: str = None):
    ...
    if checkpointer_path is None:
        checkpointer_path = os.path.join(
            settings.DATA_DIR, "checkpoints", "study_flow.db"
        )
...
def start_study_flow(user_question: str, thread_id: str, index_name: str) -> dict:
    """
    启动新的学习工作流
...
def submit_answers(thread_id: str, user_answers: dict) -> dict:
    ...
    app.update_state(
        config, {"user_answers": user_answers, "updated_at": datetime.now().isoformat()}
    )
